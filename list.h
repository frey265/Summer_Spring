...
public:
    ...
    bool Compare(const T& i, const T& j)
    {
        if (i > j)
        {
            return true;
    ...
    ListElem<T> *quickSortRecur(ListElem<T> *head, ListElem<T> *end)
    {
        if (!head || head == end)
            return head;
        ...
        ListElem<T> *pivot = partition(head, end, &newHead, &newEnd);
    ...
    void Quick_Sort(ListElem<T> **headRef)
    {
